...
module.exports = async (req, res) => {
  ...
  if (action === 'triage' && req.method === 'POST') {
    ...
    try {
      ...
      return res.json({ ok: true, data: { conversationId: conv.id, department: conv.department_id?'auto-assigned':'unassigned', priority: conv.priority||0, contact_name: conv.contacts?.name||'Desconhecido' } });
