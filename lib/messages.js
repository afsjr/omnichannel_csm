...
async function saveIncomingMessage(payload) {
  ...
  if (payload.media_type) {
    ...
    metadata.media_mimetype = payload.media_mimetype;
    metadata.media_caption = payload.media_caption;
    metadata.media_filesize = payload.media_filesize;
  }

  if (payload.message_key) {
    metadata.message_key = payload.message_key;
    metadata.original_payload = { key: payload.message_key };
  }
...
async function updateMessageMetadata(messageId, updates) {
  ...
  const merged = { ...parseMetadata(existing?.metadata), ...updates };
